...
function get_adjacent_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	...
	if ( ! $post ) {
		...
		if($rel == $previous){
			...
			$inlink = $string . '<div class="left-button"></div>' . $inlink . '</a>';
		}
		else{
			$string = '<a href="' . get_permalink( $post ) . '" rel="' . $rel . '" style="display:flex;flex-direction:column;align-items:flex-end">';
			$inlink = str_replace( '%title', $title, $link );
			$inlink = str_replace( '%date', $date, $inlink );
			$inlink = $string . '<div class="right-button"></div>' . $inlink . '</a>';
		}
...
function get_next_posts_link( $label = null, $max_page = 0 ) {
	...
	if ( ! is_single() && ( $nextpage <= $max_page ) ) {
		...
		return '<a href="' . next_posts( $max_page, false ) . "\" $attr>" . preg_replace( '/&([^#])(?![a-z]{1,8};)/i', '&#038;$1', $label ) .'<div class="left-button"></div>'. '</a>';
	}
}
